...
namespace YRental.Controllers
{
    ...
    public class MainCarsController : ApiController
    {
        ...
        {
            return YRentDB.CarListTable.ToList();
        }
        ...
        {
            var innerJoinQuery =
                from CarTypeTable in YRentDB.CarTypeTable
                join CarListTable in YRentDB.CarListTable on CarTypeTable.Car_Type_ID equals CarListTable.Car_Type_ID
        ...
        {
            return YRentDB.CarListTable.FirstOrDefault(m => m.Car_ID == id);
        }

        // POST: api/MainCars
        public void Post([FromBody]CarListTable c1)
        {
            YRentDB.CarListTable.Add(c1);
            YRentDB.SaveChanges();
        }
        ...
        {
            CarListTable u = YRentDB.CarListTable.FirstOrDefault(f => f.Car_ID == id);
            if (u != null)
            {
        ...
        {
            CarListTable c = YRentDB.CarListTable.FirstOrDefault(c1 => c1.Car_ID == id);
            if (c != null)
            {
                YRentDB.CarListTable.Remove(c);
                YRentDB.SaveChanges();
            }
        }
    }
