...
//#region  filter choosen car type and car main data  by client 
  ...
  onChange(IdValue) {
    ...
    //to get the full string of carIdvalue and convert to get only the first letter int for real id car
     this.splitted2=IdValue.split(")",2)
    let changeStringCarType = Number(this.splitted2[0]);
    this.choosen= this.arrCarsType.filter(x => x.Car_Type_ID === changeStringCarType);
...
AddOrder(userId:number)
  {   
    ...
    this.BacKSer.addingOrderBackSer(this.CarIdFromChosenCarId ? this.CarIdFromChosenCarId : this.arrCarsList[0].Car_ID,this.start,this.end,this.end,userId,this.orderStatus
    ...
    this.totalCost ? this.totalCost : this.PricePerDay,0,this.priceDelay ? this.priceDelay : this.arrCarsType[0].Price_Delay,this.totalCost ? this.totalCost : this.PricePerDay)
  }
